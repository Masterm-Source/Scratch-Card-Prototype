...
// Preload sound
const sound = new Audio('assets/sound3.mp3');
sound.loop = false;
sound.volume = 1.0;
...
function drawMessage() {
    ...
    const message = "Diana, your hard work has built a strong foundation for an incredible future. Wishing you endless success and bold new beginnings!";
    const maxWidth = messageCanvas.width - 20;
    const lineHeight = 30;
    const words = message.split(' ');
    let line = '';
    let lines = [];
    let y = messageCanvas.height / 2 - lineHeight;
...
// Fireworks logic with faster congratulatory bursts from bottom of scratch section
function checkReveal() {
    ...
    if (intervalsPassed > lastBurstAt) {
        ...
        console.log('Firing congratulatory fireworks at interval:', intervalsPassed);
        if (typeof confetti === 'function') {
            ...
            // Second burst: White and blue accents
            setTimeout(() => {
                confetti({
                    particleCount: 20,
                    spread: 60,
                    origin: { x: canvasCenterX, y: canvasBottomY },
                    colors: ['#FFFFFF', '#00B7EB'],
                    scalar: 0.8,
                    gravity: 1.0,
                    ticks: 80,
                    angle: 90,
